...
public class TextSticker extends Sticker {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        if (isEnableStroke && staticLayout2 != null)
    ...
    public TextSticker setDrawable(@NonNull Drawable drawable) {
        ...
        textRect.set(20, 20, getWidth() - 20, getHeight()-20);
        return this;
    }
    ...
    public TextSticker resizeText() {
        ...
        if (targetTextSizePixels == minTextSizePixels && targetTextHeightPixels > availableHeightPixels) {
            ...
            //
            StaticLayout staticLayout = new StaticLayout(text, textPaintCopy, availableWidthPixels,
                    Layout.Alignment.ALIGN_NORMAL, lineSpacingMultiplier, lineSpacingExtra, false);

            // Check that we have a least one line of rendered textMeasure using a StaticLayout instance
        ...
        if (isEnableStroke) {
            ...
            outLinePaint.setTextSize(targetTextSizePixels);
            staticLayout2 = new StaticLayout(this.text, outLinePaint, textRect.width(), alignment, lineSpacingMultiplier, lineSpacingExtra, true);
        }
    ...
    public void setFont(String assetsPath) {
        Typeface plain = Typeface.createFromAsset(context.getAssets(), assetsPath);
        textPaint.setTypeface(plain);
        resizeText();
    }

    public void setTextTypeface(boolean isItalic, boolean isBold) {
        if (isItalic) {
    ...
    public void setTextShader(int drawableResShader) {
        Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), drawableResShader);
    ...
    public void setStroke(boolean isEnableStroke) {
        this.isEnableStroke = isEnableStroke;
        resizeText();
    }
